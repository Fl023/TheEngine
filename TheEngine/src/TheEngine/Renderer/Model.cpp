...
namespace TheEngine {
	...
	void Model::loadMesh(unsigned int indMesh)
	{
		...
		std::vector<float> vertices = assembleVertices(positions, normals, texUVs);
		std::vector<uint32_t> indices = getIndices(JSON["accessors"][indAccInd]);
		std::vector<Ref<Texture2D>> textures = getTextures();
		...
		meshes.push_back(Mesh(vertices, layout, indices, textures));
	}
	...
	void Model::traverseNode(unsigned int nextNode, glm::mat4 matrix)
	{
		...
		// To transform the model to the correct position, we need to flip the x and z axis
		translation.x = -translation.x;
		translation.z = -translation.z;

		static const glm::quat axisCorrection = glm::angleAxis(-0.5f * glm::pi<float>(), glm::vec3(0, 1, 0));
		rotation = axisCorrection * rotation;
	...
	std::vector<Ref<Texture2D>> Model::getTextures()
	{
		...
		for (unsigned int i = 0; i < JSON["images"].size(); i++)
		{
			...
			if (!skip)
			{
				...
				{
					Ref<Texture2D> diffuse = Texture2D::Create((fileDirectory + texPath).c_str());
					diffuse->SetType("diffuse");
				...
				{
					Ref<Texture2D> specular = Texture2D::Create((fileDirectory + texPath).c_str());
					specular->SetType("specular");
	...
	std::vector<float> Model::assembleVertices
	(
	...
	)
	{
		...
		{
			// Push back the position, normal, and texture coordinates
			// The order is: position, normal, color, texture coordinates
			// The color is set to white
			vertices.insert(vertices.end(), { -positions[i].x, positions[i].y, -positions[i].z });
			vertices.insert(vertices.end(), { -normals[i].x, normals[i].y, -normals[i].z });
			vertices.insert(vertices.end(), { 1.0f, 1.0f, 1.0f });
			vertices.insert(vertices.end(), { texUVs[i].x, 1.0f - texUVs[i].y});
		}
		return vertices;
	}
	...
	std::string Model::ReadFile(const std::string& filepath)
	{
		std::string result;
